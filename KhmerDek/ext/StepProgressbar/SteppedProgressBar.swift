...
open class SteppedProgressBar: UIView {
    ...
    open var widthTitle: CGFloat = 1000 {
        didSet {
            
        }
    }
    
    open var heightTitleMultiLine: CGFloat = 50 {
        didSet {
            
        }
    }
    open var heightTitleOneLine: CGFloat = 20 {
        didSet {
    ...
    var availableFrame: CGRect {
        ...
        print(correctedFrame)
        print("00===\(screenWidth*0.22)")
    ...
    private var numberOfItems: Int {
        return stepDrawingMode == .image ? numberOfImage : titles.count
    }
    ...
    func drawTabs(from begin: Int, to end: Int, color: UIColor, textColor: UIColor) -> (start: CGPoint, end: CGPoint) {
        ...
        let x:CGFloat = startX + languageFactor * (actualSpacing + circleRadius) * CGFloat(begin)
    ...
    func drawImage(step i: Int, at rect: CGRect) {
        guard numberOfImage > i else {
            return
        }
        image?.draw(inside: rect)
    }
    ...
    func drawSuccessImage(step i: Int, at rect: CGRect) {
        if activeImage != nil {
            (tintActiveImage
                ? (activeImage?.imageWithColor(activeColor))!
                : activeImage!)
                .draw(inside: rect)
        }
    }
    ...
    func draw(step i: Int, path: UIBezierPath, start point: inout CGPoint, textColor: UIColor) {
        ...
        let numImageActive = currentTab + 1
        if numberOfImage > i || numImageActive > i {
            if (i <= currentTab - (justCheckCompleted ? 1 : 0)) {
                ...
                if image != nil {
                    (tintActiveImage
                        ? (image?.imageWithColor(activeColor))!
                        : image!)
                        .draw(inside: buttonRect)
                }
            }
        ...
        if index >= currentTab - (justCheckCompleted ? 1 : 0) || numImageActive <= index {
            //draw index
            if stepDrawingMode == .drawIndex  {
                ...
                attributedString.draw(center: point,widthTitle:widthTitle)
                self.heightTitleOneLine = self.getHeightTitleForOneLine(attributedString: attributedString)
                self.heightTitleMultiLine = self.getHeightTitleForMultieLine(attributedString: attributedString)
        ...
        if index < self.titles.count{
            ...
            attributedString.draw(center: titleCenter,widthTitle:widthTitle)
    ...
    func getHeightTitleForMultieLine(attributedString:NSAttributedString)->CGFloat{
        let drawingOptions: NSStringDrawingOptions = [
            .usesLineFragmentOrigin, .usesFontLeading]
        let rect = attributedString.boundingRect(with: CGSize(width: widthTitle, height: 1000), options: drawingOptions, context: nil)
        return rect.size.height
      
    }
    
    func getHeightTitleForOneLine(attributedString:NSAttributedString)->CGFloat{
        let drawingOptions: NSStringDrawingOptions = [
            .usesLineFragmentOrigin, .usesFontLeading]
        let rect = attributedString.boundingRect(with: CGSize(width: 1000, height: 1000), options: drawingOptions, context: nil)
          return rect.size.height
